...
// This gets an input manager for a specific platform.
// For plugged in controllers (PC/Phone) or built in controller (Game Device).
mod ffi {
    ...
    #[cfg(all(not(target_arch = "wasm32"), not(target_os = "macos"), unix))]
    include!("linux.rs");
